...
function CreateClinic() {
    ...
    const createClinicInDB = async (e: FormEvent) => {
        ...
        const { data, error } = await supabase
            .from('clinics')
            .insert({ name: formData.clinicName, welcome_message: formData.clinicWelcomeMessage, id: user.id })
            .select()
        if (error != null) {
    ...
    return (
        <form onSubmit={createClinicInDB}>
            ...
            <TextField
                onChange={(e) => setFormData((prev) => { return { ...prev, "clinicName": e.target.value } })}
                ...
                required /> <br />
            <TextField
                onChange={(e) => setFormData((prev) => { return { ...prev, "clinicWelcomeMessage": e.target.value } })}
